...
df_bx_ratings = pd.read_csv('data_prep/BX-Book-Ratings.csv', low_memory=False, encoding='unicode_escape',sep=";", quotechar='"')
...
df_bx_books = pd.read_csv('data_prep/BX-Books-fixed.csv', low_memory=False, encoding='unicode-escape',sep=';',quotechar='"', on_bad_lines='warn')

# Add cover html for link + cover image column
df_bx_books.loc[:,('Cover')]=book_recs.thumbnails(df_bx_books)

# import bx users dataset
df_bx_users = pd.read_csv('data_prep/BX-Users.csv', index_col = 'User-ID', low_memory=False, encoding='unicode_escape',sep=";", quotechar='"')
...
def data():
    ...
    if request.method == 'POST':
        ...
        book_rec_loc_isbn = book_recs.get_recs_by_loc(new_df_bx_users,new_df_bx_ratings,location)
